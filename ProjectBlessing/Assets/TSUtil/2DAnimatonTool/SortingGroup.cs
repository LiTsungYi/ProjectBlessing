...
public class SortingGroup : MonoBehaviour
{
	...
	bool Contains(Renderer renderer, SortingGroup sortingGroup)
	{
		foreach (var rendererInfo in rendererInfos)
		{
			...
			{
				if(rendererInfo.name != renderer.name)
				{
					rendererInfo.name = renderer.name;
				}
				return true;
			}
			...
			{
				if(rendererInfo.name != sortingGroup.name)
				{
					rendererInfo.name = sortingGroup.name;
				}
				return true;
			}
		}
	...
	[ContextMenu("Refresh rendererInfos")]
	public void Refresh ()
	{
	...
	[ContextMenu("ResetRenderingOrder")]
	...
	public void ForceResetRenderingOrder()
	{
		int orderIndex = GetRendererCount(this);
		SetRenderingOrder(this, ref orderIndex);
	}
	...
	public void MoveToBack (Renderer renderer)
	{
		var rendererInfo = GetRendererInfo(renderer);

		if (rendererInfo == null)
			return;

		rendererInfos.Remove (rendererInfo);
		rendererInfos.Insert (rendererInfos.Count, rendererInfo);
	}
	...
	public void MoveToFront (Renderer renderer)
	{
		...
		rendererInfos.Remove (rendererInfo);
		rendererInfos.Insert (0, rendererInfo);
	}
